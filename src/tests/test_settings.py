...
def test_settings_set_sso_enabled_trigger_nginx_regen():
    with patch("yunohost.settings.regen_conf") as regen_conf:
        settings_set("security.sso.sso_enabled", False)
        regen_conf.assert_called_once_with(names=["nginx"])
